...
class WebBuilder {
    ...
    convertToWebPath(filePath, bundleRoot = "sdat-core") {
        ...
        if (pathParts[0] === "expansion-packs") {
            ...
            // For sdat-core, common, etc., remove the first part
...
`;
    ...
    async buildExpansionPack(packName, options = {}) {
        ...
        try {
            ...
            console.warn(
                `    ⚠ No agent-teams directory found for ${packName}`
            );
        }
    }
    ...
    async buildExpansionTeamBundle(packName, packDir, teamConfigPath) {
        ...
        if (!agentsToProcess.includes("sdat-orchestrator")) {
            console.warn(
                `    ⚠ Team ${teamFileName} missing sdat-orchestrator, adding automatically`
            );
