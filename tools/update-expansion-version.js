...
if (args.length < 2) {
    console.log(
        "Usage: node update-expansion-version.js <expansion-pack-id> <new-version>"
    );
    console.log(
        "Example: node update-expansion-version.js sdat-creator-tools 1.1.0"
    );
    process.exit(1);
}
...
async function updateVersion() {
    try {
        ...
        console.log(
            `✓ Updated ${packId}/config.yaml: ${oldVersion} → ${newVersion}`
        );
        console.log(
        ...
        console.log(
            '2. Commit: git add -A && git commit -m "chore: bump ' +
                packId +
                " to v" +
                newVersion +
                '"'
