...
class V1ToV2Upgrader {
    ...
    async upgrade(options = {}) {
        try {
            ...
            // 1. Welcome message
            console.log(
                chalk.bold("\nWelcome to SDAT-Method V1 to V2 Upgrade Tool\n")
            );
            console.log(
                "This tool will help you upgrade your SDAT-Method V1 project to V2.\n"
            );
            console.log(chalk.cyan("What this tool does:"));
            console.log(
                "- Creates a backup of your V1 files (.sdat-V1-backup/)"
            );
            console.log("- Installs the new V2 .sdat-core structure");
            console.log(
            ...
            console.log("- Touch any files outside sdat-agent/ and docs/\n");
            ...
            if (!validation.isValid) {
                console.error(
                    chalk.red(
                        "\nError: This doesn't appear to be a V1 project."
                    )
                );
                console.error("Expected to find:");
                console.error("- sdat-agent/ directory");
    ...
    async showPreflightCheck(analysis, options) {
        ...
        console.log(
            `- Custom files in sdat-agent/: ${analysis.customFileCount}`
        );

        if (!options.dryRun) {
            console.log(
                "\nThe following will be backed up to .sdat-V1-backup/:"
            );
            console.log("- sdat-agent/ (entire directory)");
    ...
    async createBackup(projectPath) {
        ...
        try {
            ...
            if (await this.pathExists(backupPath)) {
                ...
                console.error(
                    chalk.red(
                        "\nError: Backup directory .sdat-V1-backup/ already exists."
                    )
                );
                console.error(
            ...
            spinner.text = "✓ Created .sdat-V1-backup/";
            console.log(chalk.green("\n✓ Created .sdat-V1-backup/"));
            ...
            console.log(chalk.green("✓ Moved sdat-agent/ to backup"));
    ...
    async installV2Structure(projectPath) {
        const spinner = ora("Installing V2 structure...").start();

        try {
            // Get the source sdat-core directory (without dot prefix)
            const sourcePath = path.join(__dirname, "..", "..", "sdat-core");
            const destPath = path.join(projectPath, ".sdat-core");

            // Copy .sdat-core
            await this.copyDirectory(sourcePath, destPath);
            spinner.text = "✓ Copied fresh .sdat-core/ directory from V2";
            console.log(
                chalk.green("\n✓ Copied fresh .sdat-core/ directory from V2")
            );
            ...
            console.log(
                chalk.yellow(
                    "\nNote: Your V1 sdat-agent content has been backed up and NOT migrated."
                )
            );
            console.log(
                chalk.yellow(
                    "The new V2 agents are completely different and look for different file structures."
                )
            );

            spinner.succeed("V2 structure installed successfully");
        } catch (error) {
            spinner.fail("V2 installation failed");
            throw error;
        }
    }
    ...
    async migrateDocuments(projectPath, analysis) {
        ...
        try {
            ...
            if (analysis.archFile) {
                ...
                console.log(
                    chalk.green(
                        `✓ Copied Architecture to docs/${analysis.archFile}`
                    )
                );
                copiedCount++;
            }
            ...
            if (analysis.frontEndArchFile) {
                ...
                console.log(
                    chalk.yellow(
                        "Note: V2 uses a single full-stack-architecture.md - use doc-migration-task to merge"
                    )
                );
                copiedCount++;
            }
            ...
            if (analysis.uxSpecFile) {
                ...
                console.log(
                    chalk.green(
                        `✓ Copied UX/UI Spec to docs/${analysis.uxSpecFile}`
                    )
                );
                copiedCount++;
            }
            ...
            if (analysis.epicFiles.length > 0) {
                ...
                console.log(
                    chalk.green(
                        "\nNote: Epic files detected! These are compatible with V2 and have been copied."
                    )
                );
                console.log(
                    chalk.green(
