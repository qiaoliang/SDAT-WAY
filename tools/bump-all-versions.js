...
async function bumpAllVersions() {
  ...
  try {
    ...
    updatedItems.push({ type: 'core', name: 'SDAT Core', oldVersion: oldCoreVersion, newVersion: newCoreVersion });
console.log(`✓ SDAT Core (package.json): ${oldCoreVersion} → ${newCoreVersion}`);
  } catch (error) {
    console.error(`✗ Failed to update SDAT Core: ${error.message}`);
  }
