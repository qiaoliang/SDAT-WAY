...
try {
    ...
    console.log(
        `Installer context not found (${e.message}), trying root context...`
    );
    try {
        version = require("../../../package.json").version;
...
program
    .version(version)
    .description(
        "SDAT Method installer - Universal AI agent framework for any domain"
    );

program
    .command("install")
    .description("Install SDAT Method agents and tools")
    .option("-f, --full", "Install complete SDAT Method")
    .option(
        "-x, --expansion-only",
        "Install only expansion packs (no sdat-core)"
    )
...
program
    .command("update")
    .description("Update existing SDAT installation")
...
╚══════╝ ╚═════╝  ╚═╝  ╚═╝    ╚═╝         ╚═╝ ╚═╝  ╚═╝  ╚═╝   ╚═╝
    ...
    const { directory } = await inquirer.prompt([
        {
            ...
            name: "directory",
            message:
                "Enter the full path to your project directory where SDAT should be installed:",
            validate: (input) => {
                if (!input.trim()) {
    ...
    const coreShortTitle =
        coreConfig["short-title"] || "SDAT Agile Core System";

    // Add SDAT core option
    let sdatOptionText;
    if (state.type === "V2_existing") {
        ...
        sdatOptionText = `Update ${coreShortTitle} ${versionInfo} .sdat-core`;
    } else {
        sdatOptionText = `${coreShortTitle} (v${version}) .sdat-core`;
    }
    ...
    // Process selections
    answers.installType = selectedItems.includes("sdat-core")
        ? "full"
        : "expansion-only";
    answers.expansionPacks = selectedItems.filter(
        (item) => item !== "sdat-core"
    );

    // Ask sharding questions if installing SDAT core
    if (selectedItems.includes("sdat-core")) {
        ...
        if (!architectureSharded) {
            ...
            console.log(
                chalk.yellow("in your core-config.yaml after installation.")
            );
            ...
            const { acknowledge } = await inquirer.prompt([
                {
                    ...
                    name: "acknowledge",
                    message:
                        "Do you acknowledge this requirement and want to proceed?",
                    default: false,
                },
            ]);
    ...
    while (!ideSelectionComplete) {
        ...
        console.log(
            chalk.bold.magenta("🔸 Use SPACEBAR to select/deselect IDEs")
        );
        console.log(
            chalk.bold.magenta("🔸 Press ENTER when finished selecting\n")
        );
    ...
    if (ides.includes("github-copilot")) {
        ...
        console.log(
            chalk.dim(
                "SDAT works best with specific VS Code settings for optimal agent experience.\n"
            )
        );
