...
class ResourceLocator {
  constructor() {
    ...
    this._sdatCorePath = null;
    this._expansionPacksPath = null;
  }

  /**
   * Get the base path for sdat-core
   */
  getSdatCorePath() {
    if (!this._sdatCorePath) {
      this._sdatCorePath = path.join(__dirname, '../../../sdat-core');
    }
    return this._sdatCorePath;
  }
  ...
  async getAgentPath(agentId) {
    ...
    // Check in sdat-core
    let agentPath = path.join(this.getSdatCorePath(), 'agents', `${agentId}.md`);
  ...
  async getAvailableAgents() {
    ...
    // Get agents from sdat-core
    const coreAgents = await this.findFiles('agents/*.md', {
      cwd: this.getSdatCorePath()
    });

    for (const agentFile of coreAgents) {
      const content = await fs.readFile(
        path.join(this.getSdatCorePath(), agentFile),
        'utf8'
      );
  ...
  async getTeamConfig(teamId) {
    ...
    const teamPath = path.join(this.getSdatCorePath(), 'agent-teams', `${teamId}.yaml`);
      ...
      for (const [type, deps] of Object.entries(dependencies)) {
        if (Array.isArray(deps)) {
          ...
          for (const dep of deps) {
            allDeps.push(`.sdat-core/${type}/${dep}`);
          }
        }
      }
  ...
  async getIdeConfig(ideId) {
    ...
    const idePath = path.join(this.getSdatCorePath(), 'ide-rules', `${ideId}.yaml`);
