...
class ConfigLoader {
  ...
  async getAvailableExpansionPacks() {
    const expansionPacksDir = path.join(this.getSdatCorePath(), '..', 'expansion-packs');
        ...
        if (entry.isDirectory() && !entry.name.startsWith('.')) {
          ...
          try {
            ...
            expansionPacks.push({
              ...
              author: config.author || 'SDAT Team',
              packPath: packPath,
  ...
  getTeamPath(teamId) {
    return path.join(this.getSdatCorePath(), 'agent-teams', `${teamId}.yaml`);
  }
